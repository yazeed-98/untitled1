...
class _FinancePageState extends State<FinancePage> {
  ...
  final List<String> _kinds = const ['بنك', 'شركة تمويل', 'محفظة إلكترونية'];
  final Set<String> _favs = {};

  Stream<QuerySnapshot<Map<String, dynamic>>> _stream() {
    final col = FirebaseFirestore.instance.collection('finance_providers');
  ...
  Widget build(BuildContext context) {
    ...
    final title = widget.cityFilter == null ? 'البنوك والتمويل' : 'التمويل — ${widget.cityFilter}';
        ...
        body: Column(
          children: [
            Padding(
              ...
              child: SearchField(
                hintText: 'ابحث باسم البنك/الشركة/المحفظة…',
              ...
              child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                ...
                builder: (context, snap) {
                  ...
                  final filtered = list.where((p) {
                    final nameOk = (p['name'] ?? '').toString().contains(_query);
                    final kindOk = _selectedKind == null || (p['kind'] ?? '') == _selectedKind;
                  ...
                  filtered.sort((a, b) {
                    switch (_sort) {
                      case 'cheap':
                        final ap = (a['feeLevel'] as num?) ?? 1;
                        final bp = (b['feeLevel'] as num?) ?? 1;
